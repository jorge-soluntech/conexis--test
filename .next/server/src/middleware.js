...
/* harmony import */ var next_dist_server_web_adapter__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(384);
        ...
        var mod = __webpack_require__(869)
...
;// CONCATENATED MODULE: ./src/consts/permissions.ts
const ACTIONS = {
    create: "create",
    edit: "edit",
    view: "view"
...
;// CONCATENATED MODULE: ./src/utils/middleware.utils.ts

const getCurrentDashboard = (pathnameSegments)=>{
    return pathnameSegments.find((word)=>word === DASHBOARDTYPES.buyer || word === DASHBOARDTYPES.supplier);
};
const getCurrentAction = (pathnameSegments)=>{
    return pathnameSegments.find((word)=>word === ACTIONS.create || word === ACTIONS.view || word === ACTIONS.edit);
};
...
async function middleware(request) {
    ...
    if (currentDashboard && currentAction) {
        const allowedRoles = PERMISSIONS[currentDashboard][currentAction];
        if (!allowedRoles.includes(role)) {
            // Redirect to the prev dashboard
            return server.NextResponse.redirect(new URL(`/dashboard`, request.url));
        }
    }
