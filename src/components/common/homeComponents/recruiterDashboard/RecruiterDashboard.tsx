...
const RecruiterDashboard = () => {
  ...
  >(
    homeUI.ITEMS_DETAILS_RECRUITER.map((data: any) => {
    ...
    const updatedCheckedState: any = checkedState.map(
      (_item: any, index: number) => {
        ...
        if (index === 0) {
          return {
            status: false,
            color: homeUI.ITEMS_DETAILS_RECRUITER[position]?.color,
          };
        }
        return {
          status: false,
          color: homeUI.ITEMS_DETAILS_RECRUITER[index]?.color,
        };
      }
    );
  ...
  const memoizedCards = useMemo(() => {
    return homeUI.ITEMS_DETAILS_RECRUITER.map((item: any, index: number) => (
