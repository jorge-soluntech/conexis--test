...
import styles from './supplierFinanceDashboard.module.scss';

const { Content } = Layout;

const SupplierFinanceDashboard = () => {
  ...
  >(
    homeUI.ITEMS_DETAILS_SUPPLIER_FINANCE.map((data: any) => {
    ...
    const updatedCheckedState: any = checkedState.map(
      (_item: any, index: number) => {
        ...
        if (index === 0) {
          return {
            status: false,
            color: homeUI.ITEMS_DETAILS_SUPPLIER_FINANCE[position]?.color,
          };
        }
        return {
          status: false,
          color: homeUI.ITEMS_DETAILS_SUPPLIER_FINANCE[index]?.color,
        };
      }
    );
  ...
  const styleHeightCards = (index: number) => {
    if (index !== 1) {
      return {
        height: checkedState[index]!.status ? 85 : 75,
        alignItems: 'center',
        transition: 'none',
      };
    }
    return {
      height: checkedState[index]!.status ? 105 : 75,
      alignItems: 'center',
      transition: 'none',
    };
  };
  ...
  const memoizedCards = useMemo(() => {
    return homeUI.ITEMS_DETAILS_SUPPLIER_FINANCE.map(
...
export default SupplierFinanceDashboard;
